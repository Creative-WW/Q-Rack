...
(() => {
  ...
  if (isIOS) {
    ...
    const intentUrl = `intent://arvr.google.com/scene-viewer/1.0?file=${encodeURIComponent(cfg.glb)}&mode=ar_preferred#Intent;scheme=https;package=com.google.android.googlequicksearchbox;action=android.intent.action.VIEW;end;`;
    arBtn.innerHTML = `<a href="${intentUrl}" aria-label="View ${cfg.title} in AR">${imgHTML}</a>`;
  } else {
